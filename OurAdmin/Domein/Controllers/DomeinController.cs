...
namespace Domein.Controllers
{
	...
	public sealed class DomeinController
	{
		...
		/// </summary>
		/// <returns>All the available servers from the serverList.</returns>
		public static List<Database> GetDatabasesFromServer() => SqlQuery("show databases;").Rows.Select(row => row.Items).Select(dbName => new Database(dbName.First().ToString())).ToList();
	}
}